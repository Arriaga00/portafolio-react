  ...
  return (
    ...
    { isVisible && (
      <div className="fixed inset-0 flex items-center justify-center z-50 transition-opacity duration-500 ease-in-out">
        ...
        <div className={`relative flex flex-col items-center max-w-lg gap-4 p-6 rounded-md shadow-md transition-transform duration-500 ease-in-out transform ${modalClass} sm:py-8 sm:px-12 bg-[#f5f5f5]`}>
          <button className="absolute top-2 right-2" onClick={()=> setAbrirModal(false)}>
            <svg
          ...
          <a
            href={Cv}
            download
            className="px-8 py-3 font-semibold rounded-full dark:bg-violet-400 dark:text-gray-900 cursor-pointer"
          >
            Descargar CV
          </a>
        </div>
